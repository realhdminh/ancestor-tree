          ...
          <div className="text-sm text-muted-foreground space-y-0.5 mt-1">
            ...
            {spouses.length > 0 && (
              <p className="flex items-center gap-1">
                ...
                {spouses.map(s => s.display_name).join(', ')}
              </p>
            )}
            ...
            {/* Biography excerpt */}
            {person.biography && <p className="mt-1 italic line-clamp-3">{person.biography}</p>}
          </div>
        </div>
      </div>
    </div>
    ...
    <div className="container mx-auto px-4 py-8 max-w-3xl">
      ...
      <div className="book-content space-y-8">
        ...
        <div className="book-cover text-center py-12 border-b-2 border-emerald-600">
          <h1 className="text-3xl md:text-4xl font-bold text-emerald-900 mb-2">Gia Phả</h1>
          <h2 className="text-xl md:text-2xl font-semibold text-emerald-700 mb-4">
            {siteConfig.clanName} {siteConfig.clanLocation}
          </h2>
          <p className="text-muted-foreground italic mb-6">&ldquo;{siteConfig.tagline}&rdquo;</p>
          ...
          <div className="space-y-1">
            {chapters.map(ch => {
              ...
              return (
                <div
                  key={ch.generation}
                  className="flex justify-between text-sm border-b border-dotted pb-1"
                >
                  <span>{ch.title}</span>
        ...
        <div className="text-center text-sm text-muted-foreground border-t pt-6">
          ...
          <p>
            Được tạo bởi {siteConfig.appTitle} · {new Date().getFullYear()}
          </p>
        </div>
      </div>
    </div>
